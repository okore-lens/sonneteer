...
import { displayErrors, optimizeImage, slugify } from "@/libs/utils";

import { ThumbnailField } from "./form";
...
const Editor = ({ post }: EditorProps) => {
	...
	const editor = useEditor({
		immediatelyRender: !articleContent,
		extensions: [
	...
	const handleResetThumbnail = () => {
		setImageFile(null);
		setImagePreview(thumbnail_url || null);
	};
	...
	const publishArticle = async () => {
		try {
			...
			if (imageFile) {
				...
				thumbnailUrl = await uploadFile(imageFile, "posts", filePath);
			}
			...
			router.push(PATHS.admin.dashboard.corner.list);
		} catch (error) {
			displayErrors(error);
		} finally {
	...
	return (
		<section id="sonn-editor" className="max-w-3xl">
			<ThumbnailField
				...
				resetThumbnail={handleResetThumbnail}
			/>
			<Textarea
				value={title}
			...
			<div className="flex flex-wrap shadow-sm mb-6 items-center gap-1 p-4 rounded-xl sticky top-0">
				<TooltipProvider>
					<Tooltip>
			...
			{getEditorHTML() &&
				isMeaningfulContent(getEditorHTML()) &&
				title && (
					<div
					...
					>
						...
						<Button loading={isPublishing} onClick={publishArticle}>
